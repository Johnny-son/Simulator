...
bool pmem_init() {
	...
	p_memory = static_cast<uint32_t *>(
		std::calloc(PHYSICAL_MEMORY_LENGTH, sizeof(uint32_t)));
	if (p_memory == nullptr) {
		return false;
	}
	g_io_words.clear();
...
bool pmem_is_ram_addr(uint32_t paddr, uint32_t size) {
	if (size == 0u || paddr < PMEM_RAM_BASE) {
		return false;
	}
	const uint64_t end =
...
uint32_t pmem_read(uint32_t paddr) {
	...
	const auto it = g_io_words.find(word_addr);
...
uint32_t *pmem_ram_ptr() { return p_memory; }
